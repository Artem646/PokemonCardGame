...
public class AquaTailAnimation : BaseAttackAnimation
{
    private GameObject trailPrefab;

    public AquaTailAnimation()
    {
        trailPrefab = Resources.Load<GameObject>("VFX/AquaTail");
    }
    ...
    private IEnumerator LaunchTailProjectile(Vector3 startPoint, Transform target, BattleCardController attacker, BattleCardController defender, AttackManager attackManager, Action<Vector3, int> showDamagePopup)
    {
        ...
        Vector3 targetPoint = target.position + Vector3.up * 0.4f + Vector3.left * 1f;
        Vector3 midPoint = Vector3.Lerp(startPoint, targetPoint, 0.5f) + Vector3.up * 1.5f + Vector3.right * 2.5f;

        yield return projectile.transform.DOPath(new Vector3[] { midPoint, targetPoint }, 0.35f, PathType.CatmullRom)
        ...
        UnityEngine.Object.Destroy(trail, 0.7f);
    }
}