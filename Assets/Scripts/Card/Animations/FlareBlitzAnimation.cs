...
public class FlareBlitzAnimation : BaseAttackAnimation
{
    private GameObject auraPrefab;
    private GameObject splashPrefab;

    public FlareBlitzAnimation()
    {
        auraPrefab = Resources.Load<GameObject>("VFX/FlareBlitzAura");
        splashPrefab = Resources.Load<GameObject>("VFX/FlareBlitzSplash");
    }
    ...
    public override IEnumerator PlayAnimation(BattleCardController attacker, BattleCardController defender, AttackManager attackManager, Action<Vector3, int> showDamagePopupCallback)
    {
        ...
        Vector3 upPos = attackerCardWrapper.transform.position - dirToDefender * 1.2f + Vector3.up * 0.8f;
        ...
        GameObject auraObject = UnityEngine.Object.Instantiate(auraPrefab, attackerCardTransform);
        auraObject.transform.SetLocalPositionAndRotation(Vector3.zero, Quaternion.identity);
        ParticleSystem aura = auraObject.GetComponentInChildren<ParticleSystem>();
        aura.Play();

        yield return attackerCardTransform.DOShakePosition(0.8f, new Vector3(0.2f, 0.2f, 0), 40, 90, false, true).WaitForCompletion();
        ...
        GameObject splashObject = UnityEngine.Object.Instantiate(splashPrefab, hitPoint, Quaternion.identity);
        ...
        showDamagePopupCallback?.Invoke(attackerCardTransform.position + Vector3.up * 1.2f, recoilDamage);

        Vector3 attackerFlyPos = hitPoint - dirToDefender * 3.5f + Vector3.up * 0.8f;
        Vector3 defenderFlyPos = defenderCardWrapper.transform.position + dirToDefender * 2.0f;
        ...
        collisionSequence.Join(defenderCardTransform.DOShakePosition(0.5f, new Vector3(0.5f, 0.5f, 0), 25));
