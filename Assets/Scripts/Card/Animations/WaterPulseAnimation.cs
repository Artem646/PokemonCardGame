...
public class WaterPulseAnimation : BaseAttackAnimation
{
    private GameObject pulsePrefab;

    public WaterPulseAnimation()
    {
        pulsePrefab = Resources.Load<GameObject>("VFX/WaterPulse");
    }
    ...
    public override IEnumerator PlayAnimation(BattleCardController attacker, BattleCardController defender, AttackManager attackManager, Action<Vector3, int> showDamagePopupCallback)
    {
        ...
        Transform exitPoint = attackerCardTransform.Find("AnimationObjects/StreamExitPoint");
        ...
        GameObject pulseObject = UnityEngine.Object.Instantiate(pulsePrefab, exitPoint);
        pulseObject.transform.LookAt(defenderCardTransform.position);
        ParticleSystem pulseParticles = pulseObject.GetComponentInChildren<ParticleSystem>();
        pulseParticles.Play();
        ...
        collisionSequence.Join(defenderCardTransform.DOShakePosition(0.5f, new Vector3(1f, 1f, 0), 40));
